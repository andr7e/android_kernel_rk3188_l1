...
static const char *goodix_ts_name = "GT9XX-TS";
...
static void gtp_pen_down(s32 x, s32 y, s32 w, s32 id)
{
	...
	input_report_abs(ts->pen_dev, ABS_MT_POSITION_X, LCD_WIDTH-x);
    input_report_abs(ts->pen_dev, ABS_MT_POSITION_Y, LCD_HEIGHT-y);
...
static s8 gtp_request_io_port(struct goodix_ts_data *ts)
{
	...
	struct gt9xx_platform_data *pdata = ts->client->dev.platform_data;
	/*struct property *pp;

	if (!np) {
		printk("%s no device tree !!! \n",__func__);
		return -EINVAL;
	}

	for (pp = np->properties; pp; pp = pp->next)printk("%s name: %s !!!!\n",__func__,pp->name);
	*/

	//irq_gpio_number = of_get_named_gpio_flags(np, "irq_gpio_number", 0, (enum of_gpio_flags *)&irq_flags);
	//rst_gpio_number = of_get_named_gpio_flags(np, "rst_gpio_number", 0, &rst_flags);

	irq_gpio_number = pdata->irq_io.gpio;
	rst_gpio_number = pdata->rst_io.gpio;
	
//	printk("%s irq_gpio_number=%d rst_gpio_number=%d -----!!\n",__func__,irq_gpio_number,rst_gpio_number);
...
static s8 gtp_request_input_dev(struct goodix_ts_data *ts)
{
    ...
    input_set_abs_params(ts->input_dev, ABS_MT_POSITION_Y, 0, ts->abs_y_max, 0, 0);  //ts->abs_x_max
...
{
    s32 ret = 0;
    printk("lml in gtx_init 11111111111\n");
    ...
    printk("lml in gtx_init 222222222222222222\n");
    return SUCCESS;
}
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    struct goodix_ts_data *ts;
    u16 version_info;
	struct gt9xx_platform_data *pdata;
    printk(" in gtx_probe 11111111111\n");
    GTP_DEBUG_FUNC();
...
#if 0
static struct of_device_id goodix_ts_ids[] = {
...
/*
    ...
    .driver = {
        .name     = GTP_I2C_NAME,
        .owner    = THIS_MODULE,
        //.of_match_table = of_match_ptr(goodix_ts_ids),
    },
};
...
MODULE_DEVICE_TABLE(i2c,goodix_ts_ids);
